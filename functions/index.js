 ...
 * [스케줄 정지 2026-09-07] 앱 공유 레이트리밋(429) — Cloud Scheduler PAUSED.
 */
...
// [스케줄 정지 2026-09-07] 순환 갭 스캔도 앱 공유 레이트리밋(429) 소진 — Cloud Scheduler PAUSED.
 ...
 * [스케줄 정지 2026-09-07] 잡 내부 Strava gap parity가 429 증폭 — Cloud Scheduler PAUSED.
 */
 ...
 * [스케줄 정지 2026-09-07] Strava gap parity 호출로 앱 레이트리밋(429) 소진 — Cloud Scheduler PAUSED.
 */
...
} = require("./vo2DemographicStats");

/** 클라이언트 Secondary relay — Firestore vo2_demographic_samples 저장 후 Supabase upsert */
exports.ingestVo2DemographicSampleRelay = onRequest(
  supabaseDualWriteServer.appendServiceRoleSecret({ cors: true, timeoutSeconds: 30 }),
  async (req, res) => {
    res.set("Access-Control-Allow-Origin", "*");
    res.set("Access-Control-Allow-Methods", "POST, OPTIONS");
    ...
    try {
      ...
      if (!uid) return;
      const body = req.body && typeof req.body === "object" ? req.body : {};
      const result = await upsertVo2DemographicSampleForUser(uid, {
        genderKey: body.genderKey,
        ageBracket: body.ageBracket,
        avgSixMonthVo2: body.avgSixMonthVo2,
      });
      if (!result.ok) {
      ...
      console.warn("[ingestVo2DemographicSampleRelay]", e.message || e);
      res.status(500).json({ success: false, error: e.message || String(e) });
    }
  }
);
...
/**
 * Firestore vo2_demographic_samples → Supabase 1회 백필
 * GET/POST ?secret=stelvio-internal-sync-v1 (또는 grade=1 Bearer)
 */
exports.backfillVo2DemographicSamplesHttp = onRequest(
  supabaseDualWriteServer.appendServiceRoleSecret({ cors: true, timeoutSeconds: 540, memory: "512MiB" }),
  async (req, res) => {
    res.set("Access-Control-Allow-Origin", "*");
    res.set("Access-Control-Allow-Methods", "GET, POST, OPTIONS");
    ...
    try {
      const r = await backfillVo2DemographicSamplesToSupabase(db);
      res.status(200).json({ success: true, ...r });
    } catch (e) {
      console.error("[backfillVo2DemographicSamplesHttp]", e.message || e);
      res.status(500).json({ success: false, error: e.message || String(e) });
    }
  }
);
...
} = require("./fitnessDemographicStats");

/** 클라이언트 Secondary relay — Firestore fitness_demographic_samples 저장 후 Supabase upsert */
exports.ingestFitnessDemographicSampleRelay = onRequest(
  supabaseDualWriteServer.appendServiceRoleSecret({ cors: true, timeoutSeconds: 30 }),
  async (req, res) => {
    res.set("Access-Control-Allow-Origin", "*");
    res.set("Access-Control-Allow-Methods", "POST, OPTIONS");
    ...
    try {
      ...
      if (!uid) return;
      const body = req.body && typeof req.body === "object" ? req.body : {};
      const result = await upsertFitnessDemographicSampleForUser(uid, {
        pmcModel: body.pmcModel,
        latestCtl: body.latestCtl,
        avgTrendCtl: body.avgTrendCtl,
        avgTrendFitness: body.avgTrendFitness,
      });
      if (!result.ok) {
        res.status(400).json({ success: false, ...result });
        return;
      ...
      console.warn("[ingestFitnessDemographicSampleRelay]", e.message || e);
      res.status(500).json({ success: false, error: e.message || String(e) });
    }
  }
);
 ...
 * GET/POST ?secret=stelvio-internal-sync-v1 (또는 grade=1 Bearer)
 */
exports.backfillFitnessDemographicSamplesHttp = onRequest(
  supabaseDualWriteServer.appendServiceRoleSecret({ cors: true, timeoutSeconds: 540, memory: "512MiB" }),
  async (req, res) => {
    res.set("Access-Control-Allow-Origin", "*");
    res.set("Access-Control-Allow-Methods", "GET, POST, OPTIONS");
    ...
    try {
      const r = await backfillFitnessDemographicSamplesToSupabase(db);
      res.status(200).json({ success: true, ...r });
    } catch (e) {
      console.error("[backfillFitnessDemographicSamplesHttp]", e.message || e);
      res.status(500).json({ success: false, error: e.message || String(e) });
    }
  }
);
