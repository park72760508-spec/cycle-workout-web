...
exports.createVerifyMeetingAttendance = createVerifyMeetingAttendance;
...
const STRAVA_RATE_LIMIT_MAX_WAIT_MS = 900000;
 ...
 */
async function fetchStravaActivityStart(accessToken, activityId) {
    ...
    for (let attempt = 0; attempt <= STRAVA_DETAIL_MAX_RETRIES; attempt++) {
        try {
            ...
            console.warn("[verifyMeetingAttendance] fetchStravaActivityStart:", e.message);
            if (attempt === STRAVA_DETAIL_MAX_RETRIES)
                return null;
        }
    }
    return null;
 ...
 */
async function fetchActivityLatLngTimeStreams(accessToken, activityId) {
    const url = `https://www.strava.com/api/v3/activities/${encodeURIComponent(activityId)}/streams?keys=latlng,time&key_by_type=true`;
    for (let attempt = 0; attempt <= STRAVA_STREAMS_MAX_RETRIES; attempt++) {
        try {
            ...
            const findData = (type) => {
                const s = streamArray.find((x) => String(x?.type || "").toLowerCase() === type);
            ...
            console.warn("[verifyMeetingAttendance] fetchActivityLatLngTimeStreams:", e.message);
            if (attempt === STRAVA_STREAMS_MAX_RETRIES)
                return null;
        }
    }
    return null;
}
function readExpiresAtSeconds(data) {
...
async function refreshStravaTokenForUser(db, userId, clientSecret) {
    ...
    if (!accessToken)
        throw new Error("Strava에서 access_token을 받지 못했습니다.");
...
async function getValidStravaAccessToken(db, userId, clientSecret) {
    const userRef = db.collection("users").doc(userId);
    ...
    if (needsRefresh) {
        try {
            ...
            console.warn(`[verifyMeetingAttendance] 토큰 갱신 실패 user=${userId}:`, e.message);
            return accessToken || null;
        }
    }
    return accessToken || null;
}
function extractMeetingLatLng(data) {
...
function extractRideDepartureLatLng(data) {
...
function getRideDateSeoulYmdFromFirestoreDate(dateField) {
    const ms = meetingTimeToMs(dateField);
    if (ms == null)
        return null;
    const d = new Date(ms);
    try {
...
function parseDepartureTimeToHourMinute(departureTime) {
 ...
 */
function combineRideMeetingTimeMs(rideData) {
...
async function findStravaActivityIdForUserOnRideDate(db, userId, rideDateYmd) {
    ...
    try {
        ...
        console.warn(`[verifyMeetingAttendance] logs 조회 실패 uid=${userId}:`, e.message);
        return null;
    }
...
function meetingTimeToMs(meetingTime) {
    if (meetingTime == null)
        return null;
    if (typeof meetingTime === "object" && meetingTime !== null && "toMillis" in meetingTime) {
        return meetingTime.toMillis();
    }
    if (typeof meetingTime === "object" && meetingTime !== null && "toDate" in meetingTime) {
        const d = meetingTime.toDate();
        return d.getTime();
    }
    if (meetingTime instanceof Date)
        return meetingTime.getTime();
    return null;
}
function wasNearMeetingStartDuringWindow(meetingMs, startMs, streams, meetLat, meetLng) {
    ...
    for (let i = 0; i < n; i++) {
        ...
        if (pointMs < windowStart || pointMs > windowEnd)
            continue;
        ...
        if (dist <= MEETING_START_RADIUS_M)
            return true;
    }
    return false;
}
 ...
 */
async function verifyOneParticipantTimeOnly(db, p, rideYmd, meetingMs, clientSecret, eventIdForDocs) {
    const baseDetail = { userId: p.userId, participantDocId: p.docId };
    const docMeta = { meetingId: eventIdForDocs, userId: p.userId };
...
async function executeVerifyAttendanceForEventId(db, eventId, clientSecretTrim, callerUid) {
    ...
    let ridesDateYmd = null; /* rides 날짜 (시각만 검증 폴백에 전달) */
    if (meetingSnap.exists) {
        ...
        const uidList = (Array.isArray(rideData.participants) ? rideData.participants : [])
            ...
            .filter(Boolean);
        const withActs = await Promise.all(uidList.map(async (userId) => {
        ...
        throw new https_1.HttpsError("not-found", "meetings 또는 rides 문서를 찾을 수 없습니다.");
    }
    if (!hostUserId) {
        throw new https_1.HttpsError("failed-precondition", "hostUserId 가 없습니다.");
    }
    if (callerUid != null && String(callerUid).trim() !== hostUserId) {
        throw new https_1.HttpsError("permission-denied", "모임 방장만 참석 검증을 실행할 수 있습니다.");
    }
    if (meetingMs == null) {
    ...
    for (const r of results) {
        ...
        if (r.detail.outcome === "ATTENDED")
            attendedCount++;
        else if (r.detail.outcome === "MISSED")
            missedCount++;
        else
            skippedCount++;
        if (r.batchUpdate)
            batchUpdates.push(r.batchUpdate);
    }
...
function createVerifyMeetingAttendance(stravaClientSecret) {
    return (0, https_1.onCall)({
        ...
        if (!clientSecret.trim()) {
            throw new https_1.HttpsError("failed-precondition", "STRAVA_CLIENT_SECRET 이 설정되지 않았습니다.");
        }
...
function createScheduledRideAttendanceVerification(stravaClientSecret) {
    return (0, scheduler_1.onSchedule)({
        ...
        try {
            ...
            console.error("[scheduledRideAttendanceVerification] rides 조회 실패:", e.message);
            return;
        }
        let ok = 0;
        let fail = 0;
        for (const doc of snap.docs) {
            const data = doc.data();
            if (data.attendanceVerificationRan === true)
                continue;
            if (String(data.rideStatus || "active") === "cancelled")
                continue;
            try {
                ...
                fail++;
                console.warn("[scheduledRideAttendanceVerification] 건너뜀/실패:", doc.id, err.message);
            }
        }
