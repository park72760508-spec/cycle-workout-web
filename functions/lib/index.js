...
exports.scheduledRideAttendanceVerification = exports.verifyMeetingAttendance = exports.onRideCreatedMeetupInviteAlimtalk = exports.meetupInviteAlimtalkHttpsRelay = exports.missionSubscriptionAlimtalkHttpsRelay = exports.onIndoorLogCreatedReward = exports.stravaWebhook = exports.naverSubscriptionSyncTest = exports.naverSubscriptionSyncSchedule = void 0;
...
const CLAIM_LAST_CHANGED_TYPES = ["CLAIM_REQUESTED", "CLAIM_COMPLETED"];
...
const aligoKakaoNatEgress_1 = require("./aligoKakaoNatEgress");
...
const smtpHost = (0, params_1.defineString)("SMTP_HOST", { default: "smtp.naver.com" });
const smtpPort = (0, params_1.defineInt)("SMTP_PORT", { default: 465 });
const adminEmail = (0, params_1.defineString)("ADMIN_EMAIL", { default: "stelvio.ai.kr@gmail.com" });
const smtpPassSecret = (0, params_1.defineSecret)("SMTP_PASS");
// Strava Webhook: 웹훅 등록 시 hub.verify_token 검증용 (.env 또는 Firebase 파라미터)
const stravaWebhookVerifyToken = (0, params_1.defineString)("STRAVA_WEBHOOK_VERIFY_TOKEN", {
...
const aligoApiKeySecret = (0, params_1.defineSecret)("ALIGO_API_KEY");
const aligoUserIdSecret = (0, params_1.defineSecret)("ALIGO_USER_ID");
const aligoTokenSecret = (0, params_1.defineSecret)("ALIGO_TOKEN");
/** Firestore 트리거 → HTTPS 릴레이 헤더 검증 (VPC는 릴레이만 적용해 알리고 IP=-99 방지) */
const meetupAlimRelaySecret = (0, params_1.defineSecret)("MEETUP_ALIM_RELAY_SECRET");
/** 릴레이 함수 URL 미지정 시 asia-northeast3-{프로젝트}.cloudfunctions.net 패턴 */
const meetupAlimRelayUrlParam = (0, params_1.defineString)("MEETUP_ALIM_RELAY_URL", { default: "" });
/** Strava 웹훅(VPC 미적용) → 미션·구독 연장 알림만 NAT 고정 출구 HTTPS 릴레이 (실내 로그 트리거와 동일 egress) */
const missionAlimRelaySecret = (0, params_1.defineSecret)("MISSION_ALIM_RELAY_SECRET");
const missionAlimRelayUrlParam = (0, params_1.defineString)("MISSION_ALIM_RELAY_URL", { default: "" });
/** 선택: 진단 공인 IP가 이 값과 다르면 경고 로그 (예: 34.64.250.77) */
const aligoNatEgressExpectIpv4 = (0, params_1.defineString)("ALIGO_NAT_EGRESS_EXPECT_IPV4", { default: "" });
/** SMTP 환경 변수를 process.env에 주입 (emailService가 읽을 수 있도록). 호출 시점에 실행 */
function injectSmtpEnv() {
    try {
...
function logLastChangedRange(range) {
    ...
    console.log("[naverSubscription] 조회 구간 요청 파라미터:", { lastChangedFrom: range.lastChangedFrom, lastChangedTo: range.lastChangedTo }, "| 간격:", intervalHours.toFixed(2), "시간 | 24시간 이내:", within24 ? "OK" : "초과(API 104140 위험)");
}
...
async function processPayedOrders(accessToken) {
    ...
    const result = await (0, naverApi_1.getLastChangedOrders)(accessToken, "PAYED", {
    ...
    console.log("[naverSubscription] lastChangedType=PAYED 조회 결과:", lastChangeStatusesLength, "건", count != null ? `(response.data.count=${count})` : "");
    if (lastChangeStatusesLength === 0) {
        console.warn("[naverSubscription] PAYED 0건. 구간:", range.lastChangedFrom, "~", range.lastChangedTo, "— naverApi 로그에서 전체 Response Body 확인");
    }
    ...
    if (productOrderIds.length > 0) {
        try {
            const details = await (0, naverApi_1.getProductOrderDetails)(accessToken, productOrderIds);
            ...
            console.warn("[naverSubscription] 주문 상세 조회 실패, last-changed 정보만으로 매칭 시도:", e.message);
        }
    }
    ...
    for (const order of orders) {
        ...
        let shippingAddressTel1 = null;
        let ordererTel = null;
        let shippingMemo = null;
        let ordererName = null;
        if (detail) {
            const extracted = (0, naverApi_1.extractContactFromDetail)(detail);
        ...
        if (!ordererTel && !shippingAddressTel1 && !shippingMemo) {
            const fromOrder = (0, naverApi_1.extractContactFromOrder)(order);
            ordererTel = fromOrder.ordererTel;
        }
        const user = await (0, subscriptionService_1.findUserByContactWithPriority)(db, shippingAddressTel1, ordererTel, shippingMemo);
        if (!user) {
            const tried1 = shippingAddressTel1 ? (0, subscriptionService_1.normalizeToContactFormat)(shippingAddressTel1) : "-";
            const tried2 = ordererTel ? (0, subscriptionService_1.normalizeToContactFormat)(ordererTel) : "-";
            const tried3 = shippingMemo ? (0, subscriptionService_1.normalizeToContactFormat)(shippingMemo) : "-";
        ...
        const { totalDays } = detail
            ? (0, naverApi_1.computeSubscriptionDaysFromProduct)(detail)
            : { totalDays: subscriptionService_1.DEFAULT_SUBSCRIPTION_DAYS };
        try {
            const { newEndDate } = await (0, subscriptionService_1.applySubscription)(db, user.userId, productOrderId, totalDays);
            const orderId = (order.orderId || "").toString();
            const productName = (detail?.productName ?? "").toString().trim() || "STELVIO AI";
            const productOptionStr = (detail?.optionManageCode ?? "").toString().trim() ||
            ...
            await (0, subscriptionService_1.saveOrderLog)(db, user.userId, productOrderId, {
                orderId,
                productOrderId,
                productName,
            ...
            console.log("[naverSubscription] 매칭 성공(" + user.priority + "순위): 유저", user.userId, "- expiry_date", newEndDate, "로 갱신 완료");
    ...
    if (matchingFailures.length > 0) {
        await (0, emailService_1.sendFailureEmail)(matchingFailures);
        for (const f of matchingFailures) {
            const tried = (f.triedNumbers ?? []).join(", ");
            console.warn("[naverSubscription] 매칭 실패 에러 리포트: productOrderId=", f.productOrderId, "| 1~3순위 연락처 변환값:", tried || "-", "| 사유:", f.reason);
        }
    }
...
function shouldRevokeByClaimDetail(claimType, claimStatus) {
    if (!claimType || !claimStatus)
        return false;
...
function claimTypeToOrderStatus(claimType) {
...
async function processRevokedOrders(accessToken, type) {
    const range = getLastChangedRange();
    logLastChangedRange(range);
    const { orders, count } = await (0, naverApi_1.getLastChangedOrders)(accessToken, type, {
        lastChangedFrom: range.lastChangedFrom,
        lastChangedTo: range.lastChangedTo,
        limitCount: 100,
    });
    console.log("[naverSubscription] lastChangedType=" + type + " 조회 결과:", orders.length, "건", count != null ? `(response.data.count=${count})` : "");
    if (orders.length === 0)
        return;
    ...
    try {
        ...
        console.warn("[naverSubscription] 클레임 상세 조회 실패:", e.message);
    }
    ...
    for (const order of orders) {
        ...
        if (!productOrderId)
            continue;
        try {
            const info = await (0, subscriptionService_1.getProcessedOrderInfo)(db, productOrderId);
            if (!info)
                continue;
            // 멱등: 이미 CANCELLED 처리된 주문은 중복 차감하지 않음 (RETURNED 포함)
            const orderLog = await (0, subscriptionService_1.getOrderLog)(db, info.userId, productOrderId);
            if (orderLog && (orderLog.status === "CANCELLED" || orderLog.status === "RETURNED")) {
                continue;
            }
            const detail = detailMap[productOrderId];
            const claimType = detail?.claimType ?? order.claimType;
            const claimStatus = detail?.claimStatus ?? order.claimStatus;
            ...
            if (revoked && userId) {
                ...
                await (0, subscriptionService_1.updateOrderLogClaim)(db, userId, productOrderId, logStatus, claimDate, `${type} 처리`);
                console.log(`[naverSubscription] 클레임 처리: 유저 ${userId}, 상태 ${claimStatus ?? "-"}, expiry_date 정정 완료`);
            }
        }
        catch (e) {
            console.error("[naverSubscription] revoke 실패:", productOrderId, e);
            await (0, emailService_1.sendRevokeFailureReport)(productOrderId, e.message);
        }
    }
}
/** 1회 동기화 실행 */
...
exports.naverSubscriptionSyncSchedule = (0, scheduler_1.onSchedule)({
    ...
    try {
        ...
        console.error("[naverSubscription] Secret 읽기 실패:", e.message, { runId });
        throw e;
    }
    if (!clientSecret.trim()) {
        console.error("[naverSubscription] NAVER_CLIENT_SECRET이 비어 있습니다. Firebase Secret(NAVER_CLIENT_SECRET) 또는 .env 확인 필요.", { runId });
        return;
    }
    try {
        ...
        if (errStack)
            console.error("[naverSubscription] stack:", errStack);
        throw err;
    }
});
...
exports.naverSubscriptionSyncTest = (0, https_1.onRequest)({
    ...
    if (smtpTest) {
        try {
            const sent = await (0, emailService_1.sendSmtpTestEmail)();
            res.status(200).json({
                success: true,
                message: sent
...
function inferWebhookRetryReason(status, errorText) {
    // 403 Application Inactive(앱 비활성)는 앱 레벨 공통 장애 — 사용자별 사유와 구분해 기록
    if (status === 403 || errorText.toLowerCase().includes("application inactive"))
        return "app_inactive";
    if (status === 401)
        return "401";
    if (status === 429 || errorText.includes("429"))
        return "429";
    if (errorText.includes("dual-write"))
        return "processing";
    return "webhook";
}
async function markWebhookStravaSyncRetryPending(db, ownerId, objectId, legacyResult, caughtError) {
    const activityId = String(legacyResult?.activityId || objectId || "").trim();
    if (!activityId)
        return;
    const errorText = String(legacyResult?.error || "") ||
...
/**
 * Webhook POST 라우터 — create/update 시 type 분기.
 *
 * [사이클 영향 검토 — 2026-06-10]
 * - Run/VirtualRun/TrailRun → processRunningActivity (Walk 제외)
 * - Ride/VirtualRide 등 → processStravaActivityAsync (기존 사이클 파이프라인)
 * - index.js processStravaActivity 본문·isCyclingForMmp·dualWriteStravaActivityLog: 미호출/미수정
 */
async function routeStravaWebhookActivityAsync(db, ownerId, objectId, _aspectType) {
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    ...
    if (preview.success &&
        ...
        try {
            ...
            console.error("[Strava Webhook] 러닝 활동 처리 실패:", runErr);
            await markWebhookStravaSyncRetryPending(db, ownerId, objectId, {
                ...
                status: runErr && typeof runErr === "object" && "status" in runErr ? Number(runErr.status) : 500,
            });
        }
        return;
    }
 ...
 */
async function handleStravaAthleteDeauthAsync(db, ownerId) {
    const ownerIdNum = Number(ownerId);
    if (!ownerIdNum)
        return;
...
async function processStravaActivityAsync(db, ownerId, objectId) {
    injectAligoEnv();
    ...
    try {
        ...
        let relaySecret = "";
        try {
            relaySecret = (0, aligoCredentials_1.scrubAligoCredential)(missionAlimRelaySecret.value());
        }
        catch {
            relaySecret = "";
        }
        const missionRelayOpts = relayUrl && relaySecret
            ? { missionAlimVpcRelay: { url: relayUrl, secret: relaySecret } }
            : undefined;
        if (!missionRelayOpts) {
            console.warn("[Strava Webhook] MISSION_ALIM_RELAY_SECRET 미설정 또는 URL 없음 — 구독 연장 알림톡은 이 함수 기본 egress로 직접 알리고 호출됩니다(카카오 API 화이트리스트 IP가 NAT 전용이면 -99 가능).");
        }
        const rewardService = new PointRewardService_1.PointRewardService(db, missionRelayOpts);
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
{
    ...
    if (req.method === "POST") {
        ...
        const isAthleteDeauth = objectType === "athlete" &&
            aspectType === "update" &&
            ownerId != null &&
            body?.updates != null &&
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
{
    ...
    if (source === "strava")
        return;
    if (logData.point_reward_v2_applied === true)
        return;
    ...
    const hasClientMileageMeta = logData.subscription_extended_days != null || logData.subscription_expiry_date_before != null;
    if (source === "stelvio" || (source === "indoor" && hasClientMileageMeta)) {
        const logId = String(event.params.logId || snap.id || "").trim();
        let historyId;
        let appendResult;
        try {
 ...
 */
exports.missionSubscriptionAlimtalkHttpsRelay = (0, https_1.onRequest)(
// eslint-disable-next-line @typescript-eslint/no-explicit-any
{
    ...aligoKakaoNatEgress_1.ALIGO_KAKAO_CLOUD_FUNCTIONS_VPC_EGRESS_OPTS,
    timeoutSeconds: 120,
    ...
    let expectedRelay;
    try {
        expectedRelay = (0, aligoCredentials_1.scrubAligoCredential)(missionAlimRelaySecret.value());
    }
    catch {
    ...
    let diagSeenPublicIp;
    try {
        const dip = await (0, aligoKakaoNatEgress_1.fetchAligoKakaoEgressPublicIpDiagnostics)();
        diagSeenPublicIp = dip ?? undefined;
        (0, aligoKakaoNatEgress_1.warnIfDiagPublicIpMismatch)(dip, aligoNatEgressExpectIpv4.value(), "missionSubscriptionAlimtalkHttpsRelay");
    ...
    try {
        const cfg = await (0, aligoAlimtalkUnified_1.loadAligoAlimtalkConfig)(db, aligoAlimtalkUnified_1.ALIMTALK_TEMPLATE.MISSION_SUBSCRIPTION);
 ...
 */
exports.meetupInviteAlimtalkHttpsRelay = (0, https_1.onRequest)(
// eslint-disable-next-line @typescript-eslint/no-explicit-any
{
    ...aligoKakaoNatEgress_1.ALIGO_KAKAO_CLOUD_FUNCTIONS_VPC_EGRESS_OPTS,
    timeoutSeconds: 300,
    ...
    let expectedRelay;
    try {
        expectedRelay = (0, aligoCredentials_1.scrubAligoCredential)(meetupAlimRelaySecret.value());
    }
    catch {
    ...
    try {
        const dip = await (0, aligoKakaoNatEgress_1.fetchAligoKakaoEgressPublicIpDiagnostics)();
        diagSeenPublicIp = dip ?? undefined;
        (0, aligoKakaoNatEgress_1.warnIfDiagPublicIpMismatch)(dip, aligoNatEgressExpectIpv4.value(), "meetupInviteAlimtalkHttpsRelay");
    ...
    try {
        ...
        const diag = diagSeenPublicIp ? { diagSeenPublicIp } : {};
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
{
    ...
    let relaySecretVal = "";
    try {
        relaySecretVal = (0, aligoCredentials_1.scrubAligoCredential)(meetupAlimRelaySecret.value());
    }
    catch (eSec) {
    ...
    const pid = process.env.GCLOUD_PROJECT || process.env.GCP_PROJECT || "";
    const fallbackUrl = pid.length > 0 ? `https://asia-northeast3-${pid}.cloudfunctions.net/meetupInviteAlimtalkHttpsRelay`.trim() : "";
