...
exports.sendRevokeFailureReport = sendRevokeFailureReport;
...
function getTransporter() {
    ...
    if (!user || !pass) {
        console.warn("[emailService] SMTP 미설정: SMTP_USER, SMTP_PASS, ADMIN_EMAIL 을 설정하면 시스템 알림 메일이 발송됩니다.");
        return null;
    }
...
async function sendFailureEmail(failures) {
    ...
    const rows = failures.map((f) => {
        const tried = f.triedNumbers && f.triedNumbers.length >= 3
 ...
 */
async function sendRevokeFailureReport(productOrderId, errorMessage) {
    const occurredAt = new Date();
    const occurredStr = occurredAt.toLocaleString("ko-KR", { timeZone: "Asia/Seoul" });
    const html = `
<!DOCTYPE html>
