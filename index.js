...
async function runRankingAggregatePeakMonthly28d(db, usersSnap = null) {
  const t0 = Date.now();
  const { startStr: r28s, endStr: r28e } = getRolling28DaysRangeSeoul();
  const snap = usersSnap ?? (await db.collection("users").get());
  await markManualRankingPhaseMeta(db, "peak_monthly", "running", { r28s, r28e, userCount: snap.size });
  const allDur = await buildPeakPowerAllDurationsForRangeAllGendersOnePass(db, r28s, r28e, snap, null);
  let wrote = 0;
  for (const gender of ["all", "M", "F"]) {
  ...
  const result = { phase: "peak_monthly", wrote, ms, startStr: r28s, endStr: r28e, userCount: snap.size };
...
async function runRankingAggregatePeakMonthlyOneDuration(db, durationType, usersSnap = null) {
  ...
  const allDur = await buildPeakPowerAllDurationsForRangeAllGendersOnePass(db, r28s, r28e, snap, durationType);
  let wrote = 0;
  for (const gender of ["all", "M", "F"]) {
    const pack = allDur[gender][durationType];
    await applyPeakRankChanges(db, pack.byCategory, `peak_${durationType}_monthly_${gender}`);
