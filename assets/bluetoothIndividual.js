...
function initializeBluetoothDashboard() {
    ...
    setTimeout(() => {
        console.log('[BluetoothIndividual] 초기 UI 업데이트:', {
            ...
            connectedDevices: {
                heartRate: window.connectedDevices?.heartRate ? window.connectedDevices.heartRate.name : null,
                powerMeter: window.connectedDevices?.powerMeter ? window.connectedDevices.powerMeter.name : null,
                trainer: window.connectedDevices?.trainer ? window.connectedDevices.trainer.name : null
            }
        });
