...
/**
 * 재가입 매칭: 전화번호와 생년이 일치하고 is_active===false 인 탈퇴 계정을 Firestore에서 조회
...
async function startWithdrawnUserRejoinFlow() {
  ...
  setTimeout(function () {
    ...
    if (profile) {
      ...
      var categoryInput = document.getElementById('registerCategoryInput');
      var profileCategory = normalizeUserSportCategory(profile.category || profile.sport_category);
      if (categoryInput) {
...
function renderSettingsProfilePanel(usersToRender, viewerGrade, viewerId) {
  ...
  const canEdit =
    viewerGrade === '1' ||
    ((viewerGrade === '2' || viewerGrade === '3') && viewerId && String(user.id) === viewerId);
  const canDelete = canEdit;
  const canTogglePrivacy = !!(viewerId && String(user.id) === viewerId);
  const showDashboardBtn = viewerGrade === '1';
  ...
  if (expRaw) {
    ...
    if (diffDays < 0) expiryClass = 'is-expired';
    else if (diffDays === 0) {
      expiryClass = 'is-soon';
      expiryText += ' (D-DAY)';
    } else if (diffDays <= 7) {
  ...
  const privacyColHtml = canTogglePrivacy
    ? '<span class="avatar-row-privacy">' +
      '<label class="privacy-toggle-label">' +
      '<input type="checkbox" class="privacy-toggle-input" ' +
      (user.is_private === true ? 'checked' : '') +
      ' onchange="toggleUserPrivacy(\'' +
      user.id +
      '\', this.checked)">' +
      '<span class="privacy-toggle-slider"></span>' +
      '<span class="privacy-toggle-text">' +
      (user.is_private === true ? '비공개' : '공개') +
      '</span></label></span>'
    : '<span class="avatar-row-privacy"></span>';
  const avatarBlock = canEditAvatar
    ? '<button type="button" class="stelvio-profile-card-avatar-btn" onclick="event.stopPropagation();typeof stelvioOpenProfilePhotoPicker===\'function\'&&stelvioOpenProfilePhotoPicker(\'' +
      user.id +
      '\')" aria-label="프로필 사진 변경">' +
      '<span class="stelvio-profile-card-avatar-ring">' +
      '<img class="stelvio-profile-card-avatar-img" src="' +
      profileUrl +
      '" alt="" width="72" height="72" loading="lazy" decoding="async" data-stelvio-profile-img="' +
      user.id +
      '" />' +
      '<span class="stelvio-profile-card-avatar-camera" aria-hidden="true"></span></span></button>'
    : '<span class="stelvio-profile-card-avatar-readonly"><span class="stelvio-profile-card-avatar-ring">' +
      '<img class="stelvio-profile-card-avatar-img" src="' +
      profileUrl +
      '" alt="" width="72" height="72" loading="lazy" decoding="async" /></span></span>';

  container.innerHTML =
    '<div class="settings-profile-panel" data-user-id="' +
    user.id +
    '">' +
    '<div class="settings-profile-panel__header">' +
    '<div class="settings-profile-panel__avatar-wrap">' +
    avatarBlock +
    '</div>' +
    '<div class="settings-profile-panel__identity">' +
    '<div class="settings-profile-panel__name-row user-name user-name-with-indicators">' +
    '<span class="user-name-text"><img src="assets/img/' +
    challengeImage +
    '" alt="" class="user-name-icon"> ' +
    displayName +
    '</span>' +
    '<span class="user-name-badges" title="AI 페어링 / Strava 연결">' +
    '<span class="profile-indicator-dot" style="width:8px;height:8px;border-radius:50%;' +
    aiDot +
    '" title="AI 페어링" aria-label="AI 페어링"></span>' +
    '<span class="profile-indicator-dot" style="width:8px;height:8px;border-radius:50%;' +
    stravaDot +
    '" title="Strava 연결" aria-label="Strava 연결"></span></span></div>' +
    '<div class="settings-profile-panel__actions user-actions">' +
    (showDashboardBtn
      ? '<button type="button" class="btn-dashboard" onclick="showPerformanceDashboard(\'' +
        user.id +
        '\')" title="대시보드 보기">📊 대시보드</button>'
      : '') +
    (canEdit
      ? '<button type="button" class="btn-edit" onclick="editUser(\'' +
        user.id +
        '\')" title="수정"><img src="assets/img/edit2.png" alt="수정" style="width:20px;height:20px;display:block;" /></button>'
      : '') +
    (canDelete
      ? '<button type="button" class="btn-delete" onclick="deleteUser(\'' +
        user.id +
        '\')" title="삭제"><img src="assets/img/delete2.png" alt="삭제" style="width:20px;height:20px;display:block;" /></button>'
      : '') +
    '</div></div></div>' +
    '<div class="settings-profile-panel__points stelvio-user-points-with-avatar settings-profile-panel__points-row">' +
    '<span class="point-badge point-accumulated" title="누적 포인트"><span class="point-icon">⭐</span><span class="point-value">' +
    formatPoints(accPoints) +
    '</span></span>' +
    '<span class="point-badge point-remaining" title="보유 포인트"><span class="point-icon">💎</span><span class="point-value">' +
    formatPoints(remPoints) +
    '</span></span>' +
    privacyColHtml +
    '</div>' +
    '<div class="settings-profile-panel__stats user-stats">' +
    '<span class="stat">FTP: ' +
    (user.ftp || '-') +
    'W</span>' +
    '<span class="stat">체중: ' +
    (user.weight || '-') +
    'kg</span>' +
    '<span class="stat">W/kg: ' +
    wkg +
    '</span></div>' +
    '<div class="settings-profile-panel__meta user-meta">' +
    '<span class="contact">' +
    (user.contact || '') +
    '</span>' +
    '<span class="expiry ' +
...
var CYCLE_CHALLENGE_OPTIONS = [
  ...
  { value: 'GranFondo', label: 'GranFondo (그란폰도)' },
  { value: 'IronMan', label: 'IronMan (아이언맨)' },
  { value: 'Racing', label: 'Racing (레이싱)' },
  { value: 'Elite', label: 'Elite (엘리트 선수)' },
  { value: 'PRO', label: 'PRO (프로 선수)' }
];
...
function populateChallengeSelectForCategory(selectEl, category, selectedValue) {
  if (!selectEl) return;
  var opts = normalizeUserSportCategory(category) === USER_SPORT_CATEGORY_RUN
    ? RUN_CHALLENGE_OPTIONS
    : CYCLE_CHALLENGE_OPTIONS;
...
function applyUserCategoryFormState(formKey, category, selectedChallenge, selectedRunChallenge) {
  ...
  if (ftpEl) {
    if (!needsFtp) {
      ...
      ftpEl.style.background = '#f0f0f0';
    } else {
      ftpEl.disabled = false;
      ftpEl.setAttribute('required', 'required');
      ftpEl.style.background = '';
      if (String(ftpEl.value) === '0') ftpEl.value = '';
    }
  }

  if (weightEl) {
    weightEl.min = isRun ? '0' : (formKey === 'register' ? '40' : '30');
  }
}

function onUserCategoryChange(selectEl) {
  if (!selectEl) return;
  var formKey = selectEl.getAttribute('data-form-key') || 'register';
  applyUserCategoryFormState(formKey, selectEl.value);
}

function isRunSportCategory(category) {
  return normalizeUserSportCategory(category) === USER_SPORT_CATEGORY_RUN;
}

if (typeof window !== 'undefined') {
  window.applyUserCategoryFormState = applyUserCategoryFormState;
...
function showAddUserForm(clearForm = true) {
  ...
  if (clearForm) {
    ...
    if (categoryEl) categoryEl.value = USER_SPORT_CATEGORY_CYCLE;
    applyUserCategoryFormState('user', USER_SPORT_CATEGORY_CYCLE, 'Fitness');
...
async function saveUser() {
  ...
  if (isRun) {
    ...
    showToast('올바른 체중을 입력해주세요. (30-200kg)'); return;
  }
...
async function editUser(userId) {
  try {
    ...
    const fillFormData = () => {
      ...
      const categoryEl = document.getElementById('editUserCategory');
...
async function completeUserInfo() {
  ...
  if (isRun) {
    if (weight == null || weight < 0 || weight > 200) {
      showToast('올바른 체중을 입력해주세요. (0-200kg)');
      return;
    }
  } else if (!weight || weight < 30 || weight > 200) {
