...
(function () {
  ...
  function RunningRankingScreen() {
    ...
    /* 종합탭 + 관리자 전용 — "전체 랭킹 + 펼쳐보기" 표시 설정(CYCLE GC와 동일 모달, platform='run') */
    var isRankingSettingsAdmin = typeof window.stelvioRankingLoginIsAdmin === 'function' && window.stelvioRankingLoginIsAdmin();
    var settingsGear = (isOverallTab && isRankingSettingsAdmin)
      ? React.createElement('button', {
          type: 'button',
          className: 'stelvio-rank-settings-gear-btn',
          'aria-label': '랭킹 표시 설정',
          title: '랭킹 표시 설정',
          onClick: function () {
            if (typeof window.openStelvioRankingDisplaySettingsModal === 'function') {
              window.openStelvioRankingDisplaySettingsModal('run');
            }
          }
        },
