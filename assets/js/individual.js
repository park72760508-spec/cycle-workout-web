...
function showTrainingResultModal(status = null) {
    ...
    let tss = 0;
    let np = 0;
    const wKgIndiv = (Number(window.currentUser && window.currentUser.weight) > 0)
      ? Number(window.currentUser.weight)
      : (window.STELVIO_RTSS_DEFAULT_WEIGHT_KG || 70);
    function tssFromRtssIndiv(elapsedSec, avgP, npVal, userFtp) {
      ...
      if (typeof window.calculateStelvioRevisedTSS === 'function' && ftpN > 0 && dur > 0 && npN > 0) {
        return window.calculateStelvioRevisedTSS(dur, ap, npN, ftpN, wKgIndiv);
      }
    ...
    if (window.trainingMetrics && window.trainingMetrics.elapsedSec > 0) {
        ...
        if (count > 0 && np4sum > 0) {
            ...
            const avgP = elapsedSec > 0 && window.trainingMetrics.joules
              ...
              : (stats.avgPower || np);
            tss = tssFromRtssIndiv(elapsedSec, avgP, np, userFtp);
            console.log('[showTrainingResultModal] TSS (trainingMetrics, rTSS):', { elapsedSec, np, tss, userFtp });
        }
    }
    ...
    if (!tss || tss === 0) {
        ...
        tss = tssFromRtssIndiv(timeForTss, avgP, np, userFtp);
        console.log('[showTrainingResultModal] TSS (대체, rTSS):', { totalSeconds, duration_min, timeForTss, np, tss, userFtp, avgPower: stats.avgPower });
    }
