...
function generateGaugeTicks() {
    ...
    // 각도: 180도(왼쪽 상단, 0)에서 90도(위쪽)를 거쳐 0도(오른쪽 상단, 6)까지 180도 범위
    // 주눈금 간격: 180도 / 6 = 30도
    ...
    for (let i = 0; i <= 24; i++) { // 0~24 (주눈금 7개 + 보조눈금 18개 = 총 25개)
        ...
        // 각도 계산: 180도에서 시작하여 90도를 거쳐 0도까지 (위쪽 반원)
        // i=0 → 180도 (왼쪽 상단), i=12 → 90도 (위쪽), i=24 → 0도 (오른쪽 상단)
        // 180도에서 시작하여 90도를 거쳐 0도로 가는 경로 (총 180도 범위)
        // 각도가 감소하는 방향: 180 → 90 → 0
        const angle = 180 - (i / 24) * 180; // 180도에서 시작하여 0도까지
...
function generateGaugeLabels() {
    ...
    multipliers.forEach((item, i) => {
        // 각도 계산: 180도에서 0도까지 (위쪽 반원)
        // i=0 → 180도 (왼쪽 상단), i=6 → 0도 (오른쪽 상단)
