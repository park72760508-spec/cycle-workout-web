...
(function () {
  ...
  var MARKET_SERVICE_URL = './marketService.js?v=20260913alertKeywordSettingsCreate1';
  var svc = null;
  ...
  var MARKET_TERMS_FULL_TEXT =
    ...
    'STELVIO(이하 "회사")는 개인 간 자전거 및 러닝 관련 중고 상품의 거래를 위한 온라인 플랫폼(장소) 및 결제 중개 시스템만을 제공할 뿐, 통신판매의 당사자가 아닙니다.\n\n' +
   ...
   * 상태 배지(.market-badge--sold, #64748b)와 같은 톤으로 보이도록 맞춘다 — 이 배지는
   * drop-shadow + 톱니 테두리 때문에 판매완료 배지와 같은 #64748b를 그대로 써도 실제로는
   * 더 진하게 보여, 그만큼 밝힌 색을 쓴다. 20% 초과~50% 미만은 원래 20% 이하에 쓰이던
   * #64748b를 그대로 물려받는다. */
  function marketCardDiscountBadgeFill(pct) {
    if (pct >= 50) return '#475569';
    if (pct <= 20) return '#94a3b8';
    return '#64748b';
  }
   ...
   * 카테고리 탭을 선택한다. CYCLE·CYCLE+RUN → 라이더 장터, RUN → 러너 장터.
