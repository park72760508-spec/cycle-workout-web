...
window.mobileTrainingState = window.mobileTrainingState || {
  ...
  /** 세그먼트 인덱스 → 해당 구간 진입 시점의 벽시계 elapsedSec(건너뛰기 직후 랩 전용) */
  _lapBaselineBySegIndex: {}
};
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // 프로필 목록은 userManager.loadUsers + renderProfileUserCards만 사용 (중복 loadUsers 제거)
...
function showDynamicFtpResultModalForMyCareer(result, userProfile) {
  ...
  if (result && result.success) {
    ...
    buttons.innerHTML = '<button type="button" onclick="closeDynamicFtpResultModalWithCooldown();" style="flex:1;min-height:48px;padding:12px 20px;font-size:15px;font-weight:600;background:#f1f5f9;color:#475569;border:1px solid #e2e8f0;border-radius:10px;cursor:pointer;touch-action:manipulation;-webkit-tap-highlight-color:transparent;position:relative;z-index:1;display:flex;align-items:center;justify-content:center">취소</button><button type="button" onclick="handleDynamicFtpModalUpdate();" style="flex:1;min-height:48px;padding:12px 20px;font-size:15px;font-weight:600;color:#fff;background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);border:none;border-radius:10px;cursor:pointer;box-shadow:0 2px 8px rgba(102,126,234,0.35);touch-action:manipulation;-webkit-tap-highlight-color:transparent;position:relative;z-index:1;display:flex;align-items:center;justify-content:center">FTP 업데이트</button>';
  } else {
    ...
    buttons.innerHTML = '<button type="button" onclick="closeDynamicFtpResultModalWithCooldown();" style="width:100%;min-height:48px;padding:12px 24px;font-size:15px;font-weight:600;color:#fff;background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);border:none;border-radius:10px;cursor:pointer;box-shadow:0 2px 8px rgba(102,126,234,0.35);touch-action:manipulation;-webkit-tap-highlight-color:transparent;position:relative;z-index:1;display:flex;align-items:center;justify-content:center">확인</button>';
  }
...
function checkAndShowDynamicFtpForMyCareer() {
  console.log('[MyCareer/FTP] checkAndShowDynamicFtpForMyCareer 시작');
  ...
  if (!user || !userId) {
    console.log('[MyCareer/FTP] 중단: 사용자 없음 (user:', !!user, ', userId:', userId, ')');
    return;
  }
  if (isDynamicFtpCooldownActive(userId)) {
    console.log('[MyCareer/FTP] 중단: 10일 보이지 않음 쿨다운 중');
    return;
  }
  if (typeof window.calculateDynamicFtp !== 'function') {
    console.warn('[MyCareer/FTP] 중단: calculateDynamicFtp 함수 없음');
    return;
  }
  console.log('[MyCareer/FTP] 조건 통과 - 비동기 FTP 산출 시작 (userId:', String(userId).slice(0, 8) + '...)');
  (async function() {
    try {
      ...
      while (pollMs < maxPollMs) {
        if (typeof window.getUserTrainingLogs === 'function' && window.firestoreV9) {
          console.log('[MyCareer/FTP] 의존성 준비됨 (대기', pollMs, 'ms)');
          break;
        }
      ...
      if (typeof window.getUserTrainingLogs === 'function' && window.firestoreV9) {
        try {
          console.log('[MyCareer/FTP] getUserTrainingLogs 호출 중...');
          logs = await window.getUserTrainingLogs(userId, { limit: 400 });
          logs = Array.isArray(logs) ? logs : [];
          console.log('[MyCareer/FTP] 로그 조회 완료:', logs.length, '건');
        } catch (fetchErr) {
          console.warn('[MyCareer/FTP] getUserTrainingLogs 1차 실패:', fetchErr && fetchErr.message);
          if (typeof window.ensureFirestoreV9ReadyForJournal === 'function') {
            await window.ensureFirestoreV9ReadyForJournal(3000);
            try {
              logs = await window.getUserTrainingLogs(userId, { limit: 400 });
              logs = Array.isArray(logs) ? logs : [];
              console.log('[MyCareer/FTP] 재시도 후 로그:', logs.length, '건');
            } catch (e2) {
      ...
      if (logs.length === 0) {
        console.log('[MyCareer/FTP] 중단: 훈련 로그 없음');
        return;
      }
      var result = window.calculateDynamicFtp(logs);
      if (!result || !result.success || result.newFtp == null) {
        console.log('[MyCareer/FTP] 중단: FTP 산출 실패 또는 결과 없음 (success:', result && result.success, ', newFtp:', result && result.newFtp, ')');
        return;
      }
      var savedFtp = Number(user.ftp || 0) || 0;
      if (result.newFtp === savedFtp) {
        console.log('[MyCareer/FTP] 중단: FTP 동일 (저장:', savedFtp, ', 산출:', result.newFtp, ')');
        return;
      }
      console.log('[MyCareer/FTP] 모달 표시 (저장 FTP:', savedFtp, '→ 산출 FTP:', result.newFtp, ')');
      showDynamicFtpResultModalForMyCareer(result, user);
    } catch (e) {
      console.warn('[MyCareer/FTP] 동적 FTP 산출 오류:', e);
    }
  })();
}
  ...
  switch(screenId) {
    ...
    case 'myCareerScreen':
      ...
      if (typeof window.checkAndShowDynamicFtpForMyCareer === 'function') {
        ...
        console.warn('[MyCareer] checkAndShowDynamicFtpForMyCareer 함수 없음');
      }
      break;
...
function buildHistoryWithTSSRuleByDate(logs) {
  ...
  for (var j = 0; j < dates.length; j++) {
    ...
    var stravaLogs = arr.filter(function (l) { return String(l.source || '').toLowerCase() === 'strava'; }).filter(isCyclingForTss);
...
/** AI selectedCategory — "(Z2)" 등 Zone 괄호 제거·표준 카테고리명 정규화 */
function stelvioCleanAiSelectedCategory(raw) {
  if (raw == null) return '';
  var s = String(raw).trim();
  s = s.replace(/\s*\(Z[^)]*\)/gi, '').replace(/\s*\[Z[^\]]*\]/gi, '').trim();
...
/** basis·AI·규칙 엔진 카테고리 → getWorkoutCategoryId 표준명 */
function stelvioNormalizeWorkoutCategoryId(cat) {
  if (!cat) return '';
  var cleaned = stelvioCleanAiSelectedCategory(cat);
  var shortMap = {
    Recovery: 'Active Recovery',
    SweetSpot: 'Sweet Spot',
    VO2Max: 'VO2 Max',
  ...
  return cleaned || String(cat).trim();
}

function stelvioCategoryIdsEqual(a, b) {
  if (!a && !b) return true;
  if (!a || !b) return false;
  return stelvioNormalizeWorkoutCategoryId(a) === stelvioNormalizeWorkoutCategoryId(b);
}

/** 대시보드 코치 recommended_workout / AI selectedCategory → 카테고리·Zone */
function stelvioParseCoachBasisRecommendedWorkout(basisRaw) {
  var raw = basisRaw != null ? String(basisRaw).trim() : '';
  ...
  if (!category && zone) {
    ...
    category = zoneToCat[zone] || '';
  }
  var displayCat = category ? stelvioNormalizeWorkoutCategoryId(category) : stelvioCleanAiSelectedCategory(raw);
  var label = displayCat && zone ? displayCat + ' (' + zone + ')' : raw;
  return { category: category, zone: zone, label: label, displayCategory: displayCat };
}

/** 코치·AI 추천 타입에 맞는 Zone 내 3순위 workout_type 문자열 (대시보드 템플릿용) */
function stelvioSameZoneWorkoutTypeLadder(zone) {
  var z = String(zone || '').trim();
  var ladders = {
    Z1: ['Active Recovery (Z1)', 'Active Recovery (Z1)', 'Active Recovery (Z1)'],
    Z2: ['Endurance (Z2)', 'Endurance (Z2)', 'Endurance (Z2)'],
    Z3: ['Tempo (Z3)', 'Tempo (Z3)', 'Tempo (Z3)'],
    'Z3~Z4': ['Sweet Spot (Z3-Z4)', 'Sweet Spot (Z3-Z4)', 'Sweet Spot (Z3-Z4)'],
    Z4: ['Threshold Intervals (Z4)', 'Threshold Intervals (Z4)', 'Threshold Intervals (Z4)'],
    Z5: ['VO2 Max (Z5)', 'VO2max Intervals (Z5)', 'VO2 Max (Z5)'],
  };
  return ladders[z] || ladders.Z2;
}

function stelvioWorkoutListId(w) {
  var id = Number(w && (w.id != null ? w.id : w.workout_id != null ? w.workout_id : w.workoutId));
  return id > 0 ? id : 0;
}

/** 목록 API 항목 → 상세·TSS·Zone 매칭용 최소 워크아웃 객체(getWorkout 실패·모바일 타임아웃 폴백) */
function stelvioListItemAsWorkoutDetail(item) {
  if (!item) return null;
  var id = stelvioWorkoutListId(item);
  if (!id) return null;
  var o = Object.assign({}, item);
  o.id = id;
  if (!Array.isArray(o.segments)) o.segments = Array.isArray(item.segments) ? item.segments.slice() : [];
  if (!o.total_seconds && item.totalSeconds != null) o.total_seconds = item.totalSeconds;
  if (!o.total_seconds && item.total_seconds != null) o.total_seconds = item.total_seconds;
  if (typeof window.ensureWorkoutAuthorZoneTag === 'function') {
    o.author = window.ensureWorkoutAuthorZoneTag(o);
  } else if (!o.author) {
    o.author = String(item.author || item.category || '').trim();
  }
  return o;
}

/** strict Zone 매칭 실패 시 VO2·카테고리·제목 기반 Z5 등 완화 매칭 */
function stelvioWorkoutMatchesTargetZoneRelaxed(workout, targetZone, basisCategory) {
  if (!workout || !targetZone) return !targetZone;
  if (stelvioWorkoutMatchesTargetZone(workout, targetZone)) return true;
  var z = String(targetZone).toUpperCase();
  var normBasis = stelvioNormalizeWorkoutCategoryId(basisCategory || '');
  if (z === 'Z5') {
    if (normBasis === 'VO2 Max' || basisCategory === 'VO2Max') {
      if (typeof window.getWorkoutCategoryId === 'function') {
        if (window.getWorkoutCategoryId(workout) === 'VO2 Max') return true;
      }
      var rawCat = String((workout.category || workout.workout_category || '') + '').trim();
      if (/vo2/i.test(rawCat)) return true;
    }
    var blob = [workout.author, workout.category, workout.title, workout.name, workout.description]
      .filter(Boolean)
      .join(' ');
    if (typeof window.textIndicatesZ5Vo2MaxCategory === 'function' && window.textIndicatesZ5Vo2MaxCategory(blob)) {
      return true;
    }
  }
  if (z === 'Z4' && (normBasis === 'Threshold' || /\bthreshold\b/i.test(String(workout.author || workout.title || '')))) {
    return true;
  }
  if (z === 'Z3' && (normBasis === 'Tempo' || /\btempo\b/i.test(String(workout.author || workout.title || '')))) {
    return true;
  }
  if (z === 'Z2' && (normBasis === 'Endurance' || /\bendurance\b/i.test(String(workout.author || workout.title || '')))) {
    return true;
  }
  if (z === 'Z1' && (normBasis === 'Active Recovery' || /recovery/i.test(String(workout.author || workout.title || '')))) {
    return true;
  }
  return false;
}

/** 타겟 Zone 워크아웃 수집 — strict 우선, 없으면 완화 매칭 */
function stelvioCollectWorkoutsForTargetZone(list, targetZone, basisCategory) {
  var arr = list || [];
  if (!targetZone) return stelvioSortWorkoutsByTssAsc(arr);
  var strict = arr.filter(function (w) {
    return stelvioWorkoutMatchesTargetZone(w, targetZone);
  });
  if (strict.length > 0) return stelvioSortWorkoutsByTssAsc(strict);
  var relaxed = arr.filter(function (w) {
    return stelvioWorkoutMatchesTargetZoneRelaxed(w, targetZone, basisCategory);
  });
  return stelvioSortWorkoutsByTssAsc(relaxed);
}

/** GAS getWorkoutsByCategory — 모바일은 배치·타임아웃·목록 폴백 */
async function stelvioFetchWorkoutDetailsResilient(workoutIds, listItems, opts) {
  opts = opts || {};
  var batchSize = opts.batchSize != null ? opts.batchSize : 5;
  var timeoutMs = opts.timeoutMs != null ? opts.timeoutMs : 14000;
  var listById = {};
  ...
  var details = [];
  var seen = new Set();
  var ids = (workoutIds || []).map(function (id) { return Number(id); }).filter(function (id) { return id > 0; });

  async function fetchOne(workoutId) {
    var fallback = stelvioListItemAsWorkoutDetail(listById[workoutId]);
    var base = window.GAS_URL;
    if (!base) return fallback;
    var ctl = typeof AbortController !== 'undefined' ? new AbortController() : null;
    var timer =
      ctl && typeof setTimeout === 'function'
        ? setTimeout(function () {
            try {
              ctl.abort();
            } catch (eab) {}
        ...
        : null;
    try {
      var params = new URLSearchParams({ action: 'getWorkout', id: String(workoutId) });
      ...
      if (result && result.success && result.item) return result.item;
    } catch (eFetch) {
      if (timer != null) clearTimeout(timer);
    }
    return fallback;
  ...
  for (var b = 0; b < ids.length; b += batchSize) {
    ...
    batchResults.forEach(function (item) {
      var d = stelvioListItemAsWorkoutDetail(item);
      if (!d) return;
      var did = stelvioWorkoutListId(d);
...
async function stelvioFetchWorkoutsByCategoryNames(categoryNames) {
  ...
  try {
    ...
    var res = await fetch(window.GAS_URL + '?' + params.toString());
    var result = await res.json().catch(function () {
      return null;
    });
    if (result && result.success && Array.isArray(result.items)) return result.items;
  } catch (eCat) {
    console.warn('[AI 추천] 카테고리 워크아웃 조회 실패:', eCat && eCat.message);
  }
  return [];
}

/** 상세 조회 대상 워크아웃 — 타겟 Zone 일치 항목 우선(최대 15) */
function stelvioSelectWorkoutsForDetailFetch(availableWorkouts, targetZone, opts) {
  opts = opts || {};
  var list = availableWorkouts || [];
  var getTitle = opts.getTitle || function (w) { return String(w.title || ''); };
  var isLite = opts.isLite || function () { return false; };
  var challenge = String(opts.challenge || '').trim();
  var basisCategory = opts.basisCategory || '';
  ...
  if (challenge === 'Fitness' && list.length > 0) {
    var liteList = list.filter(function (w) { return isLite(w); });
    var nonLiteList = list.filter(function (w) { return !isLite(w); });
    return liteList.slice(0, 5).concat(nonLiteList).slice(0, 15);
  }
  return list.slice(0, 15);
}

/** "VO2 Max", "Active Recovery (Z1)" 등 → Z5, Z1 (카테고리명 검색 금지, Zone만 사용) */
function stelvioExtractZoneTagFromText(text) {
  if (typeof window.extractZoneTagFromCategoryOrText === 'function') {
    return window.extractZoneTagFromCategoryOrText(text) || '';
  }
  if (!text) return '';
  var normCat = stelvioNormalizeWorkoutCategoryId(text);
  if (typeof window.workoutCategoryIdToZoneTag === 'function') {
    return window.workoutCategoryIdToZoneTag(normCat) || '';
  }
  return '';
}

/** AI selectedCategory 원문에서 표시용 카테고리 + 필터용 Zone 분리 */
function stelvioParseAiCategoryAndZone(raw) {
  var rawStr = raw != null ? String(raw).trim() : '';
  var zone =
    stelvioExtractZoneTagFromText(rawStr) ||
    (/\(Z[^)]+\)/i.test(rawStr) ? stelvioExtractZoneTagFromText(rawStr.match(/\(Z[^)]+\)/i)[0]) : '');
  var category = stelvioCleanAiSelectedCategory(rawStr);
  if (!zone && category) zone = stelvioExtractZoneTagFromText(category);
  return { category: category, zone: zone };
}

function stelvioResolveTargetCategoryZone(basisCategory, selectedCategory, targetZoneHint) {
  var zone = String(targetZoneHint || '').trim();
  if (!zone && selectedCategory) zone = stelvioExtractZoneTagFromText(selectedCategory);
  if (!zone && basisCategory) zone = stelvioExtractZoneTagFromText(basisCategory);
  var normCat =
    stelvioNormalizeWorkoutCategoryId(selectedCategory) ||
    stelvioNormalizeWorkoutCategoryId(basisCategory) ||
    'Endurance';
  if (!zone) {
    var basisZoneMap = {
      Recovery: 'Z1',
  ...
  if (!zone && typeof window.workoutCategoryIdToZoneTag === 'function') {
    zone = window.workoutCategoryIdToZoneTag(normCat) || zone;
  }
  if (!zone && normCat) zone = stelvioExtractZoneTagFromText(normCat);
  return { category: normCat, zone: zone };
}

function stelvioGetWorkoutAuthorForZoneMatch(w) {
  return typeof window.ensureWorkoutAuthorZoneTag === 'function'
    ? window.ensureWorkoutAuthorZoneTag(w)
    : String((w && w.author) || '');
}

function stelvioWorkoutMatchesTargetZone(workout, targetZone) {
  if (!targetZone) return true;
  if (typeof window.workoutMatchesTargetZoneTag === 'function') {
    return window.workoutMatchesTargetZoneTag(workout, targetZone);
  }
  return stelvioWorkoutAuthorMatchesTargetZone(stelvioGetWorkoutAuthorForZoneMatch(workout), targetZone);
}

function stelvioEstimateWorkoutTssSafe(w) {
  return typeof window.estimateWorkoutTSS === 'function'
    ? window.estimateWorkoutTSS(w)
    : estimateWorkoutTSS(w);
}

function stelvioSortWorkoutsByTssAsc(list) {
  return (list || []).slice().sort(function (a, b) {
    return (Number(stelvioEstimateWorkoutTssSafe(a)) || 0) - (Number(stelvioEstimateWorkoutTssSafe(b)) || 0);
  });
}
...
function stelvioFilterWorkoutPool(workoutDetails, usedIds, targetCategory, targetZone, mode) {
  var zone =
    ...
    '';
  return (workoutDetails || []).filter(function (w) {
    var wid = Number(w.id);
    if (!wid || usedIds.has(wid)) return false;
    if (!zone) return mode === 'any';
    if (mode === 'strict' || mode === 'zone' || mode === 'category') {
      if (stelvioWorkoutMatchesTargetZone(w, zone)) return true;
      if (zone && stelvioWorkoutMatchesTargetZoneRelaxed(w, zone, targetCategory)) return true;
      return false;
    }
    return true;
  });
...
function stelvioAppendFromPool(out, pool, targetCategory, targetZone, reasonPrefix) {
  var used = new Set();
  var i;
  for (i = 0; i < out.length; i++) {
    var id = Number(out[i].workoutId);
    if (id > 0) used.add(id);
  }
...
function stelvioRemapInvalidRecommendationIds(recs, workoutDetails, targetCategory, targetZone) {
  ...
  var strictPool = stelvioSortWorkoutsByTssAsc(
    stelvioFilterWorkoutPool(details, new Set(), targetCategory, targetZone, 'strict')
  );
  if (!strictPool.length) {
    strictPool = stelvioSortWorkoutsByTssAsc(
      stelvioFilterWorkoutPool(details, new Set(), targetCategory, targetZone, 'zone')
    );
  }
  if (!strictPool.length) {
    strictPool = stelvioSortWorkoutsByTssAsc(
  ...
  var used = new Set();
  var ri;
  for (ri = 0; ri < (recs || []).length && out.length < 3; ri++) {
    var r = recs[ri];
    var rid = r.workoutId != null ? Number(r.workoutId) : null;
    var slot = r.rank != null ? Number(r.rank) : out.length + 1;
    if (rid > 0 && idSet.has(rid) && !used.has(rid)) {
      used.add(rid);
    ...
    if (pick) {
      ...
      out.push({
        rank: slot,
        workoutId: Number(pick.id),
        reason: r.reason || 'Zone·카테고리에 맞춰 유사 워크아웃으로 자동 매칭: ' + (pick.title || ''),
      });
    }
  }
  out.sort(function (a, b) { return (a.rank || 0) - (b.rank || 0); });
  for (ri = 0; ri < out.length; ri++) out[ri].rank = ri + 1;
  return out.slice(0, 3);
}

/** author 텍스트 Zone 일치 (workoutManager.authorTextMatchesTargetZoneTag 위임) */
function stelvioWorkoutAuthorMatchesTargetZone(authorText, targetZone) {
  if (!targetZone) return true;
  if (typeof window.authorTextMatchesTargetZoneTag === 'function') {
    return window.authorTextMatchesTargetZoneTag(authorText, targetZone);
  }
  var a = String(authorText || '').toUpperCase();
  var z = String(targetZone).toUpperCase();
  return a.indexOf(z) >= 0;
}

/** AI recommendations 부족 시 workoutDetails에서 Zone·카테고리 맞는 항목으로 보완 (단계적 완화) */
function stelvioBackfillAiRecommendations(recs, workoutDetails, targetCategory, targetZone) {
  var normCat = stelvioNormalizeWorkoutCategoryId(targetCategory);
  var zone =
    String(targetZone || '').trim() ||
    stelvioExtractZoneTagFromText(targetCategory) ||
    stelvioExtractZoneTagFromText(normCat) ||
    '';
  if (!zone && typeof window.workoutCategoryIdToZoneTag === 'function') {
    zone = window.workoutCategoryIdToZoneTag(normCat) || '';
  }
  var out = (recs || []).slice();
  function usedIdsFromOut() {
    var u = new Set();
    var i;
    for (i = 0; i < out.length; i++) {
      var id = Number(out[i].workoutId);
  ...
  if (out.length < 3) {
    out = stelvioAppendFromPool(
      out,
      stelvioFilterWorkoutPool(workoutDetails, usedIdsFromOut(), normCat, zone, 'strict'),
      normCat,
      zone,
      '자동 보완: '
    );
  ...
  if (out.length < 3) {
    out = stelvioAppendFromPool(
      out,
      stelvioFilterWorkoutPool(workoutDetails, usedIdsFromOut(), normCat, zone, 'zone'),
      normCat,
      zone,
      'Zone 기준 보완: '
    );
  ...
  if (out.length < 3 && !zone) {
    out = stelvioAppendFromPool(
      out,
      stelvioFilterWorkoutPool(workoutDetails, usedIdsFromOut(), normCat, zone, 'any'),
      normCat,
      zone,
      '후보 풀 보완: '
    );
...
/** 최종 추천 — 타겟 Zone 불일치 항목 제거·Zone 풀만으로 3개 채움 */
function stelvioEnforceRecommendationsZoneOnly(recs, workoutDetails, targetZone, basisCategory) {
  if (!targetZone) return recs || [];
  var out = (recs || []).filter(function (r) {
    var w = (workoutDetails || []).find(function (wd) { return Number(wd.id) === Number(r.workoutId); });
    return w && stelvioWorkoutMatchesTargetZoneRelaxed(w, targetZone, basisCategory);
  });
  if (out.length < 3) {
    out = stelvioBackfillAiRecommendations(out, workoutDetails, basisCategory || '', targetZone);
  }
  return out.slice(0, 3);
}

/** 추천 3개 확보·ID 치환·백필 (실패 시 throw 없음) */
function stelvioReconcileAiRecommendations(recs, workoutDetails, ctx) {
  ctx = ctx || {};
  var resolved = stelvioResolveTargetCategoryZone(
    ctx.basisCategory,
    ctx.selectedCategory,
    ctx.targetZoneHint
  );
  var normCat = resolved.category;
  var zone = resolved.zone;
  var details = workoutDetails || [];
  if (!details.length) {
    return { recommendations: [], resolvedCategory: normCat, resolvedZone: zone, usedFallback: true };
  }

  var out = stelvioRemapInvalidRecommendationIds(recs || [], details, normCat, zone);
  out = out.filter(function (r) {
    return details.some(function (w) { return Number(w.id) === Number(r.workoutId); });
  });
  if (out.length < 3) {
    out = stelvioBackfillAiRecommendations(out, details, normCat, zone);
  }
  if (!out.length) {
    out = stelvioBackfillAiRecommendations([], details, normCat, zone);
  }
  return {
    recommendations: out.slice(0, 3),
    resolvedCategory: normCat,
    resolvedZone: zone,
    usedFallback: !(recs && recs.length),
  };
}

/** API·파싱·Zone 필터 전부 실패 시에도 화면 표시용 최소 추천 데이터 */
function stelvioBuildDeterministicRecommendationData(workoutDetails, ctx) {
  ctx = ctx || {};
  var resolved = stelvioResolveTargetCategoryZone(
    ctx.basisCategory,
    ctx.selectedCategory,
  ...
  var score = ctx.conditionScore;
  if (score == null || score < 50) score = 70;
  return {
    condition_score: Math.max(50, Math.min(100, Math.round(Number(score)))),
    training_status: ctx.trainingStatus || 'Building Base',
    vo2max_estimate: typeof ctx.vo2maxEstimate === 'number' ? ctx.vo2maxEstimate : 50,
    coach_comment:
      ctx.coachComment ||
      'AI 분석을 완료하지 못해 오늘 컨디션·훈련 부하에 맞는 후보 워크아웃을 자동으로 선정했습니다.',
    selectedCategory: resolved.category,
    target_zone: resolved.zone,
    categoryReason:
      ctx.categoryReason ||
      (resolved.zone
        ? resolved.category + ' (' + resolved.zone + ') Zone 기준 자동 추천'
...
async function analyzeAndRecommendWorkouts(date, user, apiKey, options) {
  ...
  var workoutDetailsForFallback = [];
  var availableWorkoutsForFallback = [];
  var basisCategoryForFallback = '';
  var targetZoneForFallback = '';
  var effectiveConditionScoreForFallback = null;
  var todayStrForFallback = '';
  var challengeForFallback = 'Fitness';
  ...
  try {
    ...
    challengeForFallback = challenge;
    ...
    try {
      ...
      if (result?.success && Array.isArray(result.items)) {
        ...
        availableWorkoutsForFallback = filteredWorkouts.slice();
      }
    } catch (error) {
    ...
    // 대시보드 코치 추천 Zone 선확정 → 상세 조회·AI 후보를 해당 Zone 워크아웃만 우선
    var earlyCoachBasis = stelvioParseCoachBasisRecommendedWorkout(options.basisRecommendedWorkout || '');
    var earlyTargetZone = earlyCoachBasis.zone || '';
    if (earlyCoachBasis.category) {
      basisCategoryForFallback = earlyCoachBasis.category;
      targetZoneForFallback = earlyTargetZone;
    }
    ...
    var earlyBasisCat = earlyCoachBasis.category || basisCategoryForFallback || '';
    let listForDetailFetch = stelvioSelectWorkoutsForDetailFetch(availableWorkouts, earlyTargetZone, {
      getTitle: getWorkoutTitle,
      isLite: isLiteWorkout,
      challenge: challengeNormForLite,
      basisCategory: earlyBasisCat,
    });
    if (!earlyTargetZone && needLiteFirst && availableWorkouts.length > 0) {
    ...
    } else if (!earlyTargetZone && challengeNormForLite === 'IronMan' && availableWorkouts.length > 0) {
    ...
    var zoneListForFetch = earlyTargetZone
    ...
    var workoutIds = zoneListForFetch
      .slice(0, 15)
      .map(function (w) {
        return stelvioWorkoutListId(w);
      })
      .filter(function (id) {
        return id > 0;
      });
    ...
    var isMobileFetch =
      ...
      /Android|iPhone|iPad|iPod/i.test((navigator && navigator.userAgent) || '');

    if (workoutIds.length > 0) {
      var fetchedDetails = await stelvioFetchWorkoutDetailsResilient(workoutIds, zoneListForFetch, {
        batchSize: isMobileFetch ? 4 : 6,
        timeoutMs: isMobileFetch ? 16000 : 12000,
      });
      workoutDetails.push.apply(workoutDetails, fetchedDetails);
    }

    if (workoutDetails.length < 3 && zoneListForFetch.length > 0) {
      zoneListForFetch.slice(0, 15).forEach(function (w) {
        if (workoutDetails.length >= 15) return;
        var stub = stelvioListItemAsWorkoutDetail(w);
        if (!stub) return;
        if (!workoutDetails.some(function (d) { return stelvioWorkoutListId(d) === stelvioWorkoutListId(stub); })) {
          workoutDetails.push(stub);
        }
      });
    }

    if (!workoutDetails.length && zoneListForFetch.length > 0) {
      zoneListForFetch.slice(0, 15).forEach(function (w) {
        var stubOnly = stelvioListItemAsWorkoutDetail(w);
        if (stubOnly) workoutDetails.push(stubOnly);
      });
    }

    workoutDetailsForFallback = workoutDetails.slice();
    ...
    const workoutsSummary = workoutDetails.map(function (w) {
      var authorForAi =
        typeof window.ensureWorkoutAuthorZoneTag === 'function'
          ? window.ensureWorkoutAuthorZoneTag(w)
    ...
    effectiveConditionScoreForFallback = effectiveConditionScore;
    ...
    if (basisRaw) {
      var parsedBasis = stelvioParseCoachBasisRecommendedWorkout(basisRaw);
      basisCategory = parsedBasis.category || '';
      if (parsedBasis.zone) targetZoneForFallback = parsedBasis.zone;
    } else {
    ...
    basisCategoryForFallback = basisCategory;
    ...
    const targetZoneHint = (function () {
      ...
      return basisCategory ? (categoryToTargetZone[basisCategory] || '') : '';
    })();
    targetZoneForFallback = targetZoneHint;

    var finalPromptZone = targetZoneHint || targetZoneForFallback || earlyTargetZone || '';
    var promptWorkoutPool = workoutsSummary;
    if (finalPromptZone) {
      promptWorkoutPool = workoutsSummary.filter(function (w) {
        var detail = workoutDetails.find(function (wd) { return Number(wd.id) === Number(w.id); });
        ...
        stelvioCollectWorkoutsForTargetZone(availableWorkouts, finalPromptZone, basisCategory).forEach(function (ew) {
          ...
          if (!promptWorkoutPool.some(function (p) { return Number(p.id) === ewId; })) {
            promptWorkoutPool.push({
              id: ewId,
              title: ew.title,
              author: stelvioGetWorkoutAuthorForZoneMatch(ew),
    ...
    var limitedWorkouts = promptWorkoutPool.slice(0, 10);
    if (!limitedWorkouts.length && workoutDetails.length > 0) {
      limitedWorkouts = workoutDetails.slice(0, 10).map(function (wd) {
        return {
          id: stelvioWorkoutListId(wd),
          title: wd.title || '',
          author: stelvioGetWorkoutAuthorForZoneMatch(wd),
          sec: wd.total_seconds || 0,
        };
      });
    }
    var promptCategoryLabel =
      hasBasis && basisCategory
        ? stelvioNormalizeWorkoutCategoryId(basisCategory)
        : 'Endurance';
...
[필수] 타겟 Zone "${finalPromptZone || targetZoneHint || '분석 후 결정'}" — 후보는 이 Zone의 author만 포함됨. Z2/Z3/Z4 id 선택 금지.
후보(${workoutsForPrompt.length}개, author에 Zone 표기): ${JSON.stringify(workoutsForPrompt)}
목적지침: ${challengeHintOneLine}
${hasBasis ? `고정: 표시 카테고리 ${promptCategoryLabel}, 필터 Zone "${targetZoneHint}". recommendations의 workoutId는 반드시 위 후보 id만.` : `Zone·카테고리는 부하·회복·목적 분석 후 선정. ${zoneMapLine}`}
규칙: author에 "${targetZoneHint || finalPromptZone}" 포함 후보만 선택. 카테고리명 문자열로 검색하지 말 것. workoutId 3개 모두 다름·목록에 있는 id만. reason 60자 이내. selectedCategory="${promptCategoryLabel}".
{
  ...
  "recommendations": [
    ...
    // 2순위: Flash-Lite (2.0-flash-exp 는 generateContent 미지원·단종)
    ...
    if (!recommendationData || !recommendationData.recommendations || !recommendationData.recommendations.length) {
      console.warn('[AI 추천] JSON 파싱 실패 — 결정론적 폴백 사용');
      var detailsForDeterministic = workoutDetails;
      if (!detailsForDeterministic.length && availableWorkoutsForFallback.length) {
        var zoneHintParse = targetZoneForFallback || targetZoneHint || '';
        var poolParse = zoneHintParse
          ...
          : availableWorkoutsForFallback;
        detailsForDeterministic = poolParse
          .slice(0, 15)
          .map(stelvioListItemAsWorkoutDetail)
          .filter(Boolean);
      }
      if (detailsForDeterministic.length) {
        recommendationData = stelvioBuildDeterministicRecommendationData(detailsForDeterministic, {
          basisCategory: basisCategoryForFallback,
          targetZoneHint: targetZoneForFallback,
          conditionScore: userConditionScore != null ? userConditionScore : effectiveConditionScoreForFallback,
          challenge: challengeForFallback,
        });
      } else {
        throw new Error('AI 응답을 파싱할 수 없습니다. 사용 가능한 워크아웃 후보가 없습니다.');
      }
    }
    
    // selectedCategory → 표시용 카테고리명 + 필터용 Zone(Z1~Z5) 분리
    var aiParsedCatZone = { category: '', zone: '' };
    if (recommendationData.selectedCategory != null) {
      aiParsedCatZone = stelvioParseAiCategoryAndZone(recommendationData.selectedCategory);
      recommendationData.selectedCategory = aiParsedCatZone.category;
      if (aiParsedCatZone.zone) recommendationData.target_zone = aiParsedCatZone.zone;
    }
    var effectiveTargetZone =
      recommendationData.target_zone || aiParsedCatZone.zone || targetZoneHint || '';
    ...
    if (!recommendationData.selectedCategory) {
      recommendationData.selectedCategory =
        basisCategory ||
        (deterministicDecision && deterministicDecision.category
          ? stelvioCleanAiSelectedCategory(deterministicDecision.category)
          : 'Endurance');
    }
    if (!recommendationData.target_zone) {
      recommendationData.target_zone =
        stelvioExtractZoneTagFromText(recommendationData.selectedCategory) ||
        targetZoneHint ||
        '';
    }
    effectiveTargetZone = recommendationData.target_zone || effectiveTargetZone || targetZoneHint || '';
    ...
    if (challengeNorm === 'Fitness') {
      ...
      var liteWorkouts = (workoutDetails || []).filter(function (w) { return isLite(w); });
      if (effectiveTargetZone) {
        liteWorkouts = liteWorkouts.filter(function (w) {
          return stelvioWorkoutMatchesTargetZone(w, effectiveTargetZone);
        });
      }
      ...
      if (liteIds.size > 0) {
        ...
        const nonLiteRest = restCandidates.filter(function (r) {
          ...
          if (!w || isLite(w) || liteIds.has(Number(r.workoutId))) return false;
          if (effectiveTargetZone) return stelvioWorkoutMatchesTargetZone(w, effectiveTargetZone);
          return true;
        });
        ...
        if (slot2and3.length < 2) {
          ...
          const pool = (workoutDetails || []).filter(function (w) {
            ...
            if (!liteIds.has(wid) && wid !== liteId && !usedFor2and3.has(wid) && !isLite(w)) {
              if (effectiveTargetZone) return stelvioWorkoutMatchesTargetZone(w, effectiveTargetZone);
              return true;
            }
            return false;
    ...
    var reconciled = stelvioReconcileAiRecommendations(deduped, workoutDetails, {
      basisCategory: basisCategory,
      selectedCategory: recommendationData.selectedCategory,
      targetZoneHint: effectiveTargetZone,
      conditionScore: userConditionScore != null ? userConditionScore : effectiveConditionScore,
      challenge: challengeNorm,
    });
    deduped = reconciled.recommendations;
    if (reconciled.resolvedCategory) {
      recommendationData.selectedCategory = reconciled.resolvedCategory;
    }
    if (reconciled.resolvedZone) {
      recommendationData.target_zone = reconciled.resolvedZone;
    }
    var enforceZone = effectiveTargetZone || reconciled.resolvedZone || targetZoneHint || '';
    if (enforceZone) {
      deduped = stelvioEnforceRecommendationsZoneOnly(
        deduped,
        workoutDetails,
        enforceZone,
        basisCategory || basisCategoryForFallback
    ...
    if (reconciled.usedFallback && !recommendationData.coach_comment) {
      recommendationData.coach_comment =
        'AI 추천 ID·Zone 매칭을 보완하여 오늘 훈련에 맞는 워크아웃을 자동 선정했습니다.';
    }
    if (!deduped.length) {
      var detailsForFinal = workoutDetails;
      if (!detailsForFinal.length && availableWorkoutsForFallback.length) {
        var poolFinal = enforceZone || targetZoneHint
          ? stelvioCollectWorkoutsForTargetZone(
              availableWorkoutsForFallback,
              enforceZone || targetZoneHint,
              basisCategory || basisCategoryForFallback
            )
          : availableWorkoutsForFallback;
        detailsForFinal = poolFinal
          .slice(0, 15)
          .map(stelvioListItemAsWorkoutDetail)
          .filter(Boolean);
        if (detailsForFinal.length) workoutDetails.push.apply(workoutDetails, detailsForFinal);
      }
      if (detailsForFinal.length) {
        var deterministicPack = stelvioBuildDeterministicRecommendationData(detailsForFinal, {
          basisCategory: basisCategory,
          selectedCategory: recommendationData.selectedCategory,
          targetZoneHint: targetZoneHint || enforceZone,
          conditionScore: userConditionScore != null ? userConditionScore : effectiveConditionScore,
        ...
        recommendationData = Object.assign(recommendationData, deterministicPack);
        deduped = deterministicPack.recommendations;
        console.warn('[AI 추천] Zone·author 매칭 실패 — 결정론적 3종 자동 추천');
      }
    }
    if (!deduped.length) {
    ...
    var emergencyDetails = workoutDetailsForFallback && workoutDetailsForFallback.length
      ? workoutDetailsForFallback.slice()
      : [];
    if (!emergencyDetails.length && availableWorkoutsForFallback.length) {
      var emergPool = targetZoneForFallback
        ? stelvioCollectWorkoutsForTargetZone(
            availableWorkoutsForFallback,
    ...
    if (emergencyDetails.length > 0) {
      try {
        var emergencyReco = stelvioBuildDeterministicRecommendationData(emergencyDetails, {
          basisCategory: basisCategoryForFallback,
          targetZoneHint: targetZoneForFallback,
          conditionScore:
            userConditionScore != null ? userConditionScore : effectiveConditionScoreForFallback,
          challenge: challengeForFallback,
        });
        if (typeof window.setDashboardWorkoutRecommendationCache === 'function') {
          var emergencyKey =
            (typeof window.buildLogsSignatureForCache === 'function'
              ? window.buildLogsSignatureForCache([])
              : 'fallback') +
            '_' +
            (userConditionScore != null ? String(userConditionScore) : 'auto');
          window.setDashboardWorkoutRecommendationCache(
            user.id,
            date,
            emergencyKey,
        ...
        displayWorkoutRecommendations(emergencyReco, emergencyDetails, date);
        return;
      } catch (fallbackErr) {
        console.warn('[AI 추천] 결정론적 폴백 표시 실패:', fallbackErr);
      }
    }
...
function displayWorkoutRecommendations(recommendationData, workoutDetails, date) {
  ...
  var selectedCategory = recommendationData.selectedCategory || '알 수 없음';
  var displayZone = recommendationData.target_zone || '';
  if (displayZone && selectedCategory.indexOf(displayZone) < 0 && !/\(Z/i.test(selectedCategory)) {
    selectedCategory = selectedCategory + ' (' + displayZone + ')';
  }
...
window.stelvioParseCoachBasisRecommendedWorkout = stelvioParseCoachBasisRecommendedWorkout;
window.stelvioSameZoneWorkoutTypeLadder = stelvioSameZoneWorkoutTypeLadder;
...
function startMobileTrainingTimerLoop() {
  ...
  if (typeof TrainingTimer !== 'undefined') {
    ...
    if (useFirebaseLap) {
      ...
      const calculatedSegElapsed = typeof getMobileSegmentElapsedForLap === 'function'
        ? getMobileSegmentElapsedForLap(mts, currentSegIndex)
        : Math.max(0, mts.elapsedSec - (typeof getCumulativeSegmentStartSec === 'function'
          ? getCumulativeSegmentStartSec(currentSegIndex) : 0));
    ...
    if (shouldTransition) {
      ...
      if (mts._lapBaselineBySegIndex) {
        delete mts._lapBaselineBySegIndex[currentSegIndex];
        delete mts._lapBaselineBySegIndex[nextSegIndex];
      }

      const nextSegElapsed = typeof getMobileSegmentElapsedForLap === 'function'
        ? getMobileSegmentElapsedForLap(mts, nextSegIndex)
        : Math.max(0, mts.elapsedSec - (typeof getCumulativeSegmentStartSec === 'function'
          ? getCumulativeSegmentStartSec(nextSegIndex) : 0));
      mts.segElapsedSec = nextSegElapsed;

      var nextCumLog = typeof getCumulativeSegmentStartSec === 'function' ? getCumulativeSegmentStartSec(nextSegIndex) : 0;
      console.log(`[Mobile Dashboard] 세그먼트 전환 직후 경과 시간 업데이트: segIndex=${nextSegIndex}, elapsedSec=${mts.elapsedSec}, cumStart=${nextCumLog}, segElapsedSec=${nextSegElapsed.toFixed(1)}`);
...
function refreshMobileLapTimeDisplay() {
  ...
  var elapsedInSeg = typeof getMobileSegmentElapsedForLap === 'function'
    ? getMobileSegmentElapsedForLap(mts, idx)
    : Math.max(0, (Number(mts.elapsedSec) || 0) - (typeof getCumulativeSegmentStartSec === 'function' ? getCumulativeSegmentStartSec(idx) : 0));
...
function handleMobileSkip() {
  ...
  if (currentSegIndex < w.segments.length - 1) {
    ...
    // 명목 누적(cum)과 벽시clock elapsed가 불일치할 때 랩이 00:00에 고정되지 않도록 구간 앵커(TrainingTimer SSOT와 정합)
    mts._lapBaselineBySegIndex = {};
    mts._lapBaselineBySegIndex[nextSegIndex] = Number(mts.elapsedSec) || 0;
...
/* ============================================================
 * 베이스캠프 알림 배지 (v5 — 전역 상시 onSnapshot → 주기적 폴링 전환)
 ...
 * v4까지는 로그인 세션 내내 화면과 무관하게 열려있는 onSnapshot 4종(그룹별 joinRequests
 * 팬아웃 포함)이었음 — Firestore 트래픽 절감을 위해 서버 단발 집계 엔드포인트
 * (getBasecampBadgeCountsForRead, 모임 가입신청은 Supabase 조회)를 주기 폴링하는 방식으로 대체.
 * ============================================================ */
(function () {
  'use strict';
  ...
  function _applyCountBadge(badgeId, count) {
    ...
    if (n > 0) {
      ...
      if (n >= 10) badge.setAttribute('data-wide', '1');
      else badge.removeAttribute('data-wide');
  ...
  function _renderBadges() {
    /* CYCLE 라이딩 모임 = 라이딩 초대 + 친구 요청 + 크루(내가 방장인 그룹 가입신청 대기)
       RUN 러닝 크루 = 러닝 초대 + 친구 요청 + 크루 — 화면 안 하단 네비의 라이딩·친구·클럽 배지 3개 합산(2026-08) */
    _applyCountBadge('basecampRidingNotiBadge', _counts.ridesCycle + _counts.friends + _counts.groups);
    _applyCountBadge('basecampClubHouseNotiBadge', _counts.groups);
    _applyCountBadge('runBasecampRidingNotiBadge', _counts.ridesRun + _counts.friends + _counts.groups);

    /* 카테고리 화면: 라이딩 모임 배지와 동일한 빨강원+흰숫자 배지로 표시 (친구 요청·크루 공통 + 각 종목 초대) */
    _applyCountBadge('sportCategoryCycleNotiBadge', _counts.ridesCycle + _counts.friends + _counts.groups);
    _applyCountBadge('sportCategoryRunNotiBadge', _counts.ridesRun + _counts.friends + _counts.groups);
  ...
  /* ── Firebase ID 토큰 (v9 우선, v8 compat 폴백) ── */
  function _getIdToken() {
    try {
      if (window.authV9 && window.authV9.currentUser && typeof window.authV9.currentUser.getIdToken === 'function') {
        return window.authV9.currentUser.getIdToken();
      }
    } catch (e) {}
    try {
  ...
  /* ── 단발 집계 조회 (수동 새로고침 + 주기 폴링 공용) ── */
  function _fetchBadgeCounts() {
    var uid = _getUid();
  ...
  function _onVisibilityChange() {
    if (document.visibilityState === 'visible' && _getUid()) _fetchBadgeCounts();
  }

  /* ── 폴링 시작/중지 (탭이 백그라운드일 땐 새 요청 생략, 포그라운드 복귀 시 즉시 갱신) ── */
  ...
  function _bindAuthListener() {
    function onAuthChange(user) {
      if (user || _getUid()) { _startPolling(); }
      else { _stopPolling(); _renderBadges(); }
    }
    if (typeof firebase !== 'undefined' && firebase.auth && typeof firebase.auth === 'function') {
      try {
    ...
    if (window.authV9 && typeof window.authV9.onAuthStateChanged === 'function') {
      try {
        window.authV9.onAuthStateChanged(onAuthChange);
      } catch (e2) {}
    }
    /* 폴백: 세션 복원 지연 시 폴링 */
    var elapsed = 0;
    var poll = setInterval(function () {
      elapsed += 500;
      if (_getUid()) { clearInterval(poll); _startPolling(); return; }
