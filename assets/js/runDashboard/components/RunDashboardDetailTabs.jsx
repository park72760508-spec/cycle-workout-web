/**
 * RunDashboardDetailTabs - Level 3 탭 구조
 ...
 * Tab 3: 성장 추이 (STELVIO 헥사곤(레벨) → 나의 성장 트렌드 → 훈련 부하 rTSS → VO₂max 트렌드 → 년간 파워PR)
 *
...
(function() {
  ...
  if (!window.React) {
    console.warn('[RunDashboardDetailTabs] React not loaded');
    return;
  }
  ...
  /** 프로필 카드에 있던 TP/심박 존 테이블 — 대시보드「러닝 지표」탭 상단 */
  function RunningMetricsZoneTables(props) {
    ...
    var ftp = Number(stats.thresholdPace) || (userProfile && Number(userProfile.ftp)) || 0;
    ...
    if (!html) return null;
    var wrapClass =
      'dashboard-running-metrics-zones profile-zone-tables-wrap' +
      (standalone ? ' dashboard-running-metrics-zones-standalone' : '');
  ...
  function RunDashboardDetailTabs(props) {
    var p = props || {};
    ...
    function renderTabContent() {
      ...
      if (activeIndex === 2) {
        return React.createElement(
          'div',
          { className: 'space-y-6' },
          StelvioHexagonRanksCard &&
            React.createElement(StelvioHexagonRanksCard, { userProfile: userProfile, DashboardCard: DashboardCard }),
          logsLoading ? React.createElement(
            DashboardCard,
            { title: '나의 성장 트렌드' },
            React.createElement('div', { className: 'h-[200px] flex flex-col items-center justify-center' },
      ...
      if (activeIndex === 3) {
        ...
        return React.createElement(
          'div',
          { className: 'space-y-6' },
          React.createElement(RunningMetricsZoneTables, {
            userProfile: userProfile,
            stats: stats,
            standalone: true
          }),
          React.createElement(
            ...
            React.createElement('div', { className: 'px-4 pb-4 pt-1 space-y-4 text-xs text-gray-600 border-t-0' },
              ...
              React.createElement('section', null,
                React.createElement('div', { className: 'font-semibold text-gray-800 mb-2' }, '훈련 등급(challenge) · 주간 목표 rTSS'),
                React.createElement('p', { className: 'mb-2' }, '회원가입/프로필에서 선택. 주간 목표 rTSS · RPE 보정 · 목표 조절 범위에 사용'),
                React.createElement('table', { className: 'w-full text-left border-collapse' },
                  React.createElement('thead', null,
                    React.createElement('tr', { className: 'border-b border-gray-200' },
                      React.createElement('th', { className: 'py-1 pr-2' }, '등급'),
          ...
          React.createElement(
            ...
            React.createElement('div', { className: 'px-4 py-3 text-xs text-gray-600' },
              React.createElement('div', { className: 'font-semibold text-gray-800 mb-1' }, 'STELVIO 헥사곤 등급표'),
              React.createElement(
                'p',
    ...
    return React.createElement(
      'div',
      { className: 'RunDashboardDetailTabs', ref: containerRef },
      React.createElement(
        'div',
        {
  ...
  window.RunDashboardDetailTabs = RunDashboardDetailTabs;
})();
