...
CREATE OR REPLACE FUNCTION public.fn_running_speed_from_avg_paces_3(
  ...
  p_pace2_sec double precision,
  p_pace3_sec double precision
)
...
AS $$
  SELECT CASE
    ...
    WHEN p_pace2_sec IS NULL OR p_pace2_sec <= 0 OR NOT isfinite(p_pace2_sec::numeric) THEN NULL
    WHEN p_pace3_sec IS NULL OR p_pace3_sec <= 0 OR NOT isfinite(p_pace3_sec::numeric) THEN NULL
    ELSE 1000.0 / ((p_pace1_sec + p_pace2_sec + p_pace3_sec) / 3.0)
  END;
$$;

COMMENT ON FUNCTION public.fn_running_speed_from_avg_paces_3(double precision, double precision, double precision) IS
  '세 페이스(sec/km) 산술 평균에 해당하는 속도(m/s)';

-- 단·중거리 Top3: 고유 기록 2건 — 2등×1.30 가상기록 후 3개 평균
CREATE OR REPLACE FUNCTION public.fn_running_top3_penalty_speed_2records(
...
IMMUTABLE
AS $$
  SELECT public.fn_running_speed_from_avg_paces_3(
    public.fn_running_pace_sec_per_km(p_best_speed),
    public.fn_running_pace_sec_per_km(p_second_speed),
    public.fn_running_pace_sec_per_km(p_second_speed) * 1.30
  );
$$;

COMMENT ON FUNCTION public.fn_running_top3_penalty_speed_2records(double precision, double precision) IS
  'Top3 그룹 기록 2건: (1등 + 2등 + 2등×1.30) / 3 페이스 평균 속도';

-- 단·중거리 Top3: 고유 기록 1건 — 1등×1.30 가상기록 2개 후 3개 평균
CREATE OR REPLACE FUNCTION public.fn_running_top3_penalty_speed_1record(
  p_best_speed double precision
)
RETURNS double precision
LANGUAGE sql
...
COMMENT ON FUNCTION public.fn_running_top3_penalty_speed_1record(double precision) IS
  'Top3 그룹 기록 1건: (1등 + 1등×1.30 + 1등×1.30) / 3 페이스 평균 속도';

-- 레거시 호환: 구간별 top-N 평균 — v5 get_running_leaderboard 내부 일별 집계로 대체
...
AS $$
  SELECT CASE
    ...
    WHEN lower(btrim(COALESCE(p_axis, ''))) IN ('1k', '3k', '5k', '7k') THEN
      CASE
        WHEN p_peak_count >= 3 THEN
          public.fn_running_top3_penalty_speed_2records(p_best_speed, p_second_speed)
        WHEN p_peak_count = 2 THEN
          public.fn_running_top3_penalty_speed_2records(p_best_speed, p_second_speed)
...
COMMENT ON FUNCTION public.fn_running_sliding_axis_speed(text, double precision, double precision, integer) IS
  '레거시 헬퍼 — v5 랭킹은 get_running_leaderboard 내부 일별 Top-N 집계 사용';
...
GRANT EXECUTE ON FUNCTION public.fn_running_speed_from_avg_paces_3(double precision, double precision, double precision) TO authenticated;
GRANT EXECUTE ON FUNCTION public.fn_running_speed_from_avg_paces_3(double precision, double precision, double precision) TO service_role;
GRANT EXECUTE ON FUNCTION public.fn_running_top3_penalty_speed_2records(double precision, double precision) TO authenticated;
GRANT EXECUTE ON FUNCTION public.fn_running_top3_penalty_speed_2records(double precision, double precision) TO service_role;
GRANT EXECUTE ON FUNCTION public.fn_running_top3_penalty_speed_1record(double precision) TO authenticated;
GRANT EXECUTE ON FUNCTION public.fn_running_top3_penalty_speed_1record(double precision) TO service_role;
