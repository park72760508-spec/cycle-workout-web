-- RUN 중첩 불변식: 긴 거리(speed) 존재 시 짧은 거리 누락 → 더 긴 구간 속도로 보간
-- (레거시 DB·streams 부분 실패 시 3k만 있고 1k null 문제 해결)

CREATE OR REPLACE FUNCTION public.fn_running_impute_shorter_speeds(
...
AS $$
  WITH s AS (
    SELECT
      CASE WHEN p_speed_1k  > 0 THEN p_speed_1k  END AS s1,
      CASE WHEN p_speed_3k  > 0 THEN p_speed_3k  END AS s3,
      CASE WHEN p_speed_5k  > 0 THEN p_speed_5k  END AS s5,
      CASE WHEN p_speed_7k  > 0 THEN p_speed_7k  END AS s7,
      CASE WHEN p_speed_10k > 0 THEN p_speed_10k END AS s10,
      CASE WHEN p_speed_20k > 0 THEN p_speed_20k END AS s20,
      CASE WHEN p_speed_42k > 0 THEN p_speed_42k END AS s42
  ),
  c AS (
    SELECT
      s42 AS v42,
      COALESCE(s20, s42) AS v20,
      COALESCE(s10, COALESCE(s20, s42)) AS v10,
      COALESCE(s7, COALESCE(s10, COALESCE(s20, s42))) AS v7,
      COALESCE(s5, COALESCE(s7, COALESCE(s10, COALESCE(s20, s42)))) AS v5,
      COALESCE(s3, COALESCE(s5, COALESCE(s7, COALESCE(s10, COALESCE(s20, s42))))) AS v3,
      COALESCE(
        s1,
        COALESCE(s3, COALESCE(s5, COALESCE(s7, COALESCE(s10, COALESCE(s20, s42)))))
      ) AS v1
    FROM s
  )
  SELECT v1, v3, v5, v7, v10, v20, v42 FROM c;
$$;

COMMENT ON FUNCTION public.fn_running_impute_shorter_speeds IS
  'RUN 중첩 보간: speed_3k 있으면 speed_1k 누락 시 speed_3k 로 채움 (42k→1k 연쇄)';

-- get_running_leaderboard: 활동별 단조→보간→완성도 우선 프로필 선택
...
COMMENT ON FUNCTION public.get_running_leaderboard() IS
  'RUN v2: 단조→짧은구간 보간→완성도 우선 프로필 + 6축 GC';

GRANT EXECUTE ON FUNCTION public.fn_running_impute_shorter_speeds(
