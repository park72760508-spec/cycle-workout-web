-- RUN 종합: (1) 활동 단위 중첩 프로필 1건 선택 (2) GC 6축 포지션 합산 (3) 구 스냅샷 무효화
...
AS $$
  ...
  activity_mono AS (
    ...
    WHERE COALESCE(f.speed_1k, 0) > 0
       OR COALESCE(f.speed_3k, 0) > 0
       OR COALESCE(f.speed_5k, 0) > 0
       OR COALESCE(f.speed_7k, 0) > 0
       OR COALESCE(f.speed_10k, 0) > 0
       OR COALESCE(f.speed_20k, 0) > 0
       OR COALESCE(f.speed_42k, 0) > 0
  ),
  ranked_activity AS (
    SELECT
      am.*,
...
COMMENT ON FUNCTION public.get_running_leaderboard() IS
  'RUN 랭킹 v2: 활동 1건 프로필(중첩 윈도우) + 6축 GC 포지션 합산(최대 600). peak_performances=페이스탭 max.';
...
BEGIN
  ...
  IF snap.leaderboard IS NOT NULL
    ...
    IF snap_version >= 2
       AND COALESCE(snap.leaderboard -> 0 ? 'gc_scores', false) THEN
...
COMMENT ON FUNCTION public.get_running_leaderboard_published() IS
  'RUN 랭킹 조회 — scoring_version>=2 스냅샷 우선, 구버전/없으면 live GC v2';
