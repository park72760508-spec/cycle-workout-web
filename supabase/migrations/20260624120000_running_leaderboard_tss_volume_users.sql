-- RUN 랭킹 TSS/거리: GC total_score=0 이어도 주간 TSS·30일 거리가 있으면 보드에 포함
-- (박지성 등 페이스/GC 점수 없이 activities.tss 만 있는 사용자 누락 수정)
...
IMMUTABLE
AS $$
  SELECT regexp_replace(lower(btrim(COALESCE(p_type, ''))), '[\s_-]+', '', 'g') IN (
    'run', 'virtualrun', 'trailrun'
  );
$$;

COMMENT ON FUNCTION public.fn_is_running_activity_type(text) IS
  'RUN 랭킹·TSS·거리 집계 대상 Strava activity_type (Walk 제외, 공백/언더스코어 정규화)';
...
AS $$
  ...
  all_users AS (
    ...
    WHERE weekly_tss > 0 OR distance_30d_km > 0
  ),
  adjusted AS (
    SELECT
      u.user_id,
...
COMMENT ON FUNCTION public.get_running_leaderboard() IS
  'RUN v5: 90일 GC + TSS/거리 전용 사용자 포함';
...
-- 배포 직후 published 스냅샷·TSS 보드 스냅샷 즉시 재집계
