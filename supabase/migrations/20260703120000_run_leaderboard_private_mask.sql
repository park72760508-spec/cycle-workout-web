-- RUN 랭킹보드 비공개(is_private) 처리를 CYCLE와 동일한 "포함 + 마스킹"으로 통일.
--
-- 변경 전(문제): get_running_leaderboard()/fn_run_leaderboard_score_rows() 가
--   `WHERE p.is_private = false` 로 비공개 사용자를 아예 제외 → RUN 클라이언트의 마스킹 로직이
--   전혀 동작하지 않았고, CYCLE(주간TSS·거리·종합)의 "이름만 '비공개' 마스킹" 방식과 불일치.
--
-- 변경 후:
--   1) board_users(GC 코호트)·ranked_rows 에서 is_private 제외 필터 제거 → 비공개 사용자도 순위에 포함.
--   2) user_info.display_name 을 v_user_public_profile 의 마스킹('비공개') 대신 실제 users.name 으로 내려주고,
--      is_private 플래그를 함께 전달 → 클라이언트가 CYCLE와 동일하게 비관리자에게는 마스킹, 관리자에게는 실명 노출.
--   3) fn_run_leaderboard_score_rows 의 is_private 제외 필터 제거 → 등락 스냅샷에도 비공개 사용자 포함(이름은 클라 마스킹).
--
-- ※ CYCLE 의 fn_weekly_tss_leaderboard_live / heptagon_cohort_ranks 와 동일한 정책(비공개 포함·is_private 플래그·클라 마스킹).
...
COMMENT ON FUNCTION public.get_running_leaderboard() IS
  'RUN v7: 주간 TSS + 주간 거리(월~일 오늘까지 누계) + 30일 거리. 비공개 사용자 포함(is_private 플래그·실명 전달, 클라 마스킹).';
...
AS $$
  ...
  FROM (
    ...
    WHERE r->'user_info'->>'user_id' IS NOT NULL
...
COMMENT ON FUNCTION public.fn_run_leaderboard_score_rows(jsonb, text, text, text) IS
  'RUN 지표별 점수행 — 비공개 사용자 포함(이름 마스킹은 클라이언트가 is_private로 처리).';

GRANT EXECUTE ON FUNCTION public.fn_run_leaderboard_score_rows(jsonb, text, text, text) TO service_role;

-- 배포 직후: 새 로직(비공개 포함·마스킹)으로 published 스냅샷 강제 재발행 후 등락 재집계.
