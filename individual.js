...
db.ref(`sessions/${SESSION_ID}/users/${myBikeId}`).on('value', (snapshot) => {
    ...
    if (data) {
        ...
        if (data.ftp !== undefined && data.ftp !== null) {
            ...
            console.log('[Firebase] 사용자 FTP 값:', userFTP);
...
function updateTargetPower() {
    ...
    console.log('[updateTargetPower] 세그먼트 인덱스:', currentSegmentIndex, 'target_type:', targetType, 'target_value:', targetValue, 'FTP:', ftp);
